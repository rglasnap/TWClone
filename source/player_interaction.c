...
handle_player (void *threadinfo)
{
  ...
  do
	...
	pthread_exit (NULL);
      ...
      if (strncmp (inbuffer, "DESCRIPTION", strlen("DESCRIPTION")) == 0 && loggedin)
	{
	  ...
	  commandgood = 1;
		...
		(strncmp (inbuffer, "NEW", strlen("NEW")) == 0)) && !loggedin)
	{
      ...
      else if (strncmp (inbuffer, "PLAYERINFO", strlen("PLAYERINFO")) == 0 && loggedin)
	{
	  popstring (inbuffer, temp, " ", BUFF_SIZE);
      ...
      else if (strncmp (inbuffer, "UPDATE", strlen("UPDATE")) == 0 && loggedin)
	{
	  strcpy (data.name, name);
      ...
      else if (strncmp (inbuffer, "ONLINE", strlen("ONLINE")) == 0)
	{
      ...
      else if (strncmp (inbuffer, "FEDCOMM", strlen("FEDCOMM") ) == 0 && loggedin)
	{
	  popstring (inbuffer, temp, " ", BUFF_SIZE);
	  popstring (inbuffer, temp, ":", BUFF_SIZE);
      ...
      else if (strncmp (inbuffer, "SCAN", strlen("SCAN")) == 0 && loggedin)
	{
      ...
      else if (strncmp (inbuffer, "BEACON", strlen("BEACON")) == 0 && loggedin)
	{
      ...
      else if (strncmp (inbuffer, "TOW", strlen("TOW")) == 0 && loggedin)
	{
      ...
      else if (strncmp (inbuffer, "LISTNAV", strlen("LISTNAV")) == 0 && loggedin)
	{
	  data.command = ct_listnav;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "TRANSPORTER", strlen("TRANSPORTER")) == 0 && loggedin)
	{
	  data.command = ct_transporter;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "GENESIS", strlen("GENESIS")) == 0 && loggedin)
	{
	  popstring(inbuffer, temp, " ", BUFF_SIZE);
	  popstring(inbuffer, data.buffer, ":", BUFF_SIZE);
      ...
      else if (strncmp (inbuffer, "JETTISON", strlen("JETTISON")) == 0 && loggedin)
	{
	  data.command = ct_jettison;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "INTERDICT", strlen("INTERDICT")) == 0 && loggedin)
	{
	  data.command = ct_interdict;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "ATTACK", strlen("ATTACK")) == 0 && loggedin)
	{
	  data.command = ct_attack;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "ETHERPROBE", strlen("ETHERPROBE")) == 0 && loggedin)
	{
	  data.command = ct_etherprobe;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "FIGHTERS", strlen("FIGHTERS")) == 0 && loggedin)
	{
	  data.command = ct_fighters;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "LISTFIGHTERS", strlen("LISTFIGHTERS")) == 0 && loggedin)
	{
      ...
      else if (strncmp (inbuffer, "MINES", strlen("MINES")) == 0 && loggedin)
	{
      ...
      else if (strncmp (inbuffer, "LISTMINES", strlen("LISTMINES")) == 0 && loggedin)
	{
	  data.command = ct_listmines;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "PORTUPGRADE", strlen("PORTUPGRADE")) == 0 && loggedin)
	{
	  data.command = ct_portconstruction;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "SETNAVS", strlen("SETNAVS")) == 0 && loggedin)
	{
	  data.command = ct_setnavs;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "STATUS", strlen("STATUS")) == 0 && loggedin)
	{
      ...
      else if (strncmp (inbuffer, "HAIL", strlen("HAIL")) == 0 && loggedin)
	{
      ...
      else if (strncmp (inbuffer, "SUBSPACE", strlen("SUBSPACE")) == 0 && loggedin)
	{
	  data.command = ct_subspace;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "LISTWARPS", strlen("LISTWARPS")) == 0 && loggedin)
	{
      ...
      else if (strncmp (inbuffer, "AVOID", strlen("AVOID")) == 0 && loggedin)
	{
      ...
      else if (strncmp (inbuffer, "LISTAVOIDS", strlen("LISTAVOIDS")) == 0 && loggedin)
	{
	  data.command = ct_listavoids;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "SELFDESTRUCT", strlen("SELFDESTRUCT")) == 0 && loggedin)
	{
	  data.command = ct_selfdestruct;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "LISTSETTINGS", strlen("LISTSETTINGS")) == 0 && loggedin)
	{
	  data.command = ct_listsettings;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "SETTINGS", strlen("SETTINGS")) == 0 && loggedin)
	{
	  data.command = ct_updatesettings;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "ANNOUNCE", strlen("ANNOUNCE")) == 0 && loggedin)
	{
	  data.command = ct_dailyannouncement;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "PHOTON", strlen("PHOTON")) == 0 && loggedin)
	{
	  data.command = ct_firephoton;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "READMAIL", strlen("READMAIL")) == 0 && loggedin)
	{
	  data.command = ct_readmail;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "SENDMAIL", strlen("SENDMAIL")) == 0 && loggedin)
	{
      ...
      else if (strncmp (inbuffer, "TIME", strlen("TIME")) == 0 && loggedin)
	{
      ...
      else if (strncmp (inbuffer, "DISRUPTOR", strlen("DISRUPTOR")) == 0 && loggedin)
	{
	  data.command = ct_usedisruptor;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "DAILYLOG", strlen("DAILYLOG")) == 0 && loggedin)
	{
	  data.command = ct_dailylog;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "ALIENRANKS", strlen("ALIENRANKS")) == 0 && loggedin)
	{
	  data.command = ct_alienranks;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "LISTPLAYERS", strlen("LISTPLAYERS")) == 0 && loggedin)
	{
	  data.command = ct_listplayers;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "LISTPLANETS", strlen("LISTPLANETS")) == 0 && loggedin)
	{
	  data.command = ct_listplanets;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "LISTSHIPS", strlen("LISTSHIPS")) == 0 && loggedin)
	{
	  data.command = ct_listships;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "LISTCORPS", strlen("LISTCORPS")) == 0 && loggedin)
	{
	  data.command = ct_listcorps;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "JOINCORP", strlen("JOINCORP")) == 0 && loggedin)
	{
	  data.command = ct_joincorp;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "MAKECORP", strlen("MAKECORP")) == 0 && loggedin)
	{
	  data.command = ct_makecorp;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "CREDTRANS", strlen("CREDTRANS")) == 0 && loggedin)
	{
	  data.command = ct_credittransfer;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "FIGTRANS", strlen("FIGTRANS")) == 0 && loggedin)
	{
	  data.command = ct_fightertransfer;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "MINETRANS", strlen("MINETRANS")) == 0 && loggedin)
	{
	  data.command = ct_minetransfer;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "SHIELDTRANS", strlen("SHIELDTRANS")) == 0 && loggedin)
	{
	  data.command = ct_shieldtransfer;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "CORPQUIT", strlen("CORPQUIT")) == 0 && loggedin)
	{
	  data.command = ct_quitcorp;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "CORPPLANETS", strlen("CORPPLANETS")) == 0 && loggedin)
	{
	  data.command = ct_listcorpplanets;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "ASSETS", strlen("ASSETS")) == 0 && loggedin)
	{
	  data.command = ct_showassets;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "CORPMEMO", strlen("CORPMEMO")) == 0 && loggedin)
	{
	  data.command = ct_corpmemo;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "DROPMEMBER", strlen("DROPMEMBER")) == 0 && loggedin)
	{
	  data.command = ct_dropmember;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "CORPPASS", strlen("CORPPASS")) == 0 && loggedin)
	{
	  data.command = ct_corppassword;
	  commandgood = 1;
	}
      else if (strncmp (inbuffer, "SHIPINFO", strlen("SHIPINFO")) == 0 && loggedin)
	{
	  popstring (inbuffer, temp, " ", BUFF_SIZE);
	  data.to = popint (inbuffer, ":");
      ...
      else if (strncmp (inbuffer, "QUIT", strlen("QUIT")) == 0 && loggedin)
	{
      ...
      else if (strncmp (inbuffer, "PORTINFO", strlen("PORTINFO")) == 0 && loggedin)
      ...
      else if (strncmp (inbuffer, "PORT", strlen("PORT")) == 0 && loggedin)
	{
      ...
      else if (strncmp (inbuffer, "MYINFO", strlen("MYINFO")) == 0 && loggedin)
	{
	  strcpy (data.name, name);
